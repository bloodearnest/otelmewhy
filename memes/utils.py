...
def load_impact_font(size):
    ...
    try:
        ...
        try:
            ...
            try:
                return ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVu-Sans-Bold.ttf", size)
            except (OSError, IOError):
...
def generate_meme(image_url, top_text="", bottom_text=""):
    ...
    if base_image.mode in ("RGBA", "LA"):
        ...
        background.paste(base_image, mask=base_image.split()[-1] if base_image.mode == "RGBA" else None)
    ...
    font_size = min(top_font_size, bottom_font_size) if top_text and bottom_text else max(top_font_size, bottom_font_size, 40)
    ...
    if top_text:
        ...
        draw_text_with_outline(draw, top_text.upper(), (x, y), font, outline_width=outline_width)
    ...
    if bottom_text:
        ...
        # Calculate outline width based on font size
        outline_width = max(font_size // 20, 3)  # 5% of font size, minimum 3px
        draw_text_with_outline(draw, bottom_text.upper(), (x, y), font, outline_width=outline_width)
